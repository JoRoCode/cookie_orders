...
@app.post('/cookies/new')
def new_order():
    if order.Order.create_new_order(request.form):
...
@app.get('/cookies')
def index():
    order_info = order.Order.get_all_orders()
    print(order_info)
    return render_template('dashboard.html', order_info = order_info)



# Update Users Controller

@app.get('/cookies/edit/<int:id>')
def edit_order(id):
    order_info = order.Order.get_order_by_id(id)
    return render_template('edit_order.html', order=order_info, id=id)

@app.post('/cookies/edit/<int:id>')
def return_edit_order(id):
    if order.Order.edit_one_order(request.form):
        return redirect('/cookies')
